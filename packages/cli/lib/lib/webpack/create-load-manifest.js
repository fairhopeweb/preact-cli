const isESM = filename => /(?<!legacy)\.js$/.test(filename);
const isMatch = filename => isESM(filename);
	...
	for (let filename in assets) {
		if (!/\.map$/.test(filename)) {
			if (/^route-.*\.js$/.test(filename)) {
				...
				isMatch(filename) && scripts.push(filename);
